...
import numpy as np
import yaml
# this to to diosable annoying yaml warning
...
def lidar_points2numpy(lidar_points):
    ...
    return np.delete(numpy_array, 0, 1)
...
def apply_manual_correction_to_the_projection_mtx(projection_mtx):
    assert type(
        projection_mtx) == np.ndarray, "projection matrix must be of a numpy array"
    assert projection_mtx.shape == (
        3, 3), "shape of the projeciton matrix is not 3x3"
    ...
    return np.array([[projection_mtx[0, 0], projection_mtx[0, 1], projection_mtx[0, 2] + correction_on_cu],
                     [projection_mtx[1, 0], projection_mtx[1, 1],
                         projection_mtx[1, 2]],
...
class LidarProjector:
    ...
    def __init__(self,
        ...
        if self.save_images_and_lidar_bags is not None and self.save_images_and_lidar_bags:
            ...
            self.dir_path = os.path.join(
                self.dir_path, datetime.now().strftime('%Y-%m-%d_%H-%M-%S'))
            if not os.path.exists(self.dir_path):
                self.dir_rgb = os.path.join(self.dir_path, "rgb")
                os.makedirs(self.dir_rgb)
                self.dir_lidar_points = os.path.join(
                    self.dir_path, "lidar_points")
                ...
                if self.save_depth_maps:
                    ...
                    if self.create_control:
                        self.dir_control = os.path.join(
                            self.dir_path, "control")
        ...
        self.lidar_sub = message_filters.Subscriber(
            lidar_topic2sub, PointCloud2)

        self.time_diff_between_msgs = sync_difference_time
        ...
        if self.publish_colourised_pcd_msgs:
            self.lidar_rgb_pub = rospy.Publisher(
                lidar_topic2sub+"_rgb", PointCloud2, queue_size=1)
        ...
        if 'nir' in img_topic2sub:
            ...
            tf_camera = '/pp_rgb'
        self.lidar2cam_translation20, self.lidar2cam_rotation20 = LidarProjector.get_lidar2cam_transform(
            tf.TransformListener(), tf_camera)
        self.computation_extrinsic_matrix20 = LidarProjector.get_computation_extrinsic_mtx(
            self.lidar2cam_rotation20, self.lidar2cam_translation20)
    ...
    def callback(self, ros_dist_img, lidar_points):
        ...
        if self.input_imgs_are_distorted:
            if self.mtx4undist is None:
                ...
                self.projection4undist = apply_manual_correction_to_the_projection_mtx(
                    self.projection4undist)

                # # print the projection mtx for the console
                text = "Projection matrix is: {}".format(
                    self.projection4undist)
                print(text)
                rospy.loginfo(text)
        ...
        if self.save_images_and_lidar_bags:
            ...
            dir_lidar_points = os.path.join(
                self.dir_lidar_points, str(str(self.counter)+".bag"))
            bag = rosbag.Bag(dir_lidar_points, 'w')
            try:
                bag.write(topic='velodyne_points', msg=lidar_points,
                          t=lidar_points.header.stamp)
            finally:
                bag.close()
        ...
        if self.save_depth_maps or self.create_control or self.publish_colourised_pcd_msgs:
            ...
            points3D_in_cam = np.matmul(
                self.computation_extrinsic_matrix20, points_numpy[0:4, :])
            projected_points_homogenous = np.matmul(
                self.projection4undist, points3D_in_cam)
            uv_rows = projected_points_homogenous[0:2, :].reshape(
                (2, points_numpy.shape[1]))
            w_row = projected_points_homogenous[2, :].reshape(
                (1, points_numpy.shape[1]))
            ...
            for proj_point, point_in_3D, point_numpy in zip(continous_projected_points.transpose(),
                ...
                if point_numpy[0] > 1.1 and 0 < proj_point[0] < width and 0 < proj_point[1] < height:
                    ...
                    discretized_projected_point = (
                        int(proj_point[0]), int(proj_point[1]))

                    if self.publish_colourised_pcd_msgs:
                        (r, g, b) = cv_image[discretized_projected_point[1],
                                             discretized_projected_point[0]]
                        a = 255
                        rgb = struct.unpack(
                            'I', struct.pack('BBBB', b, g, r, a))[0]
                        colourized_point = [
                            point_numpy[0], point_numpy[1], point_numpy[2], point_numpy[4], rgb]
                    ...
                    if self.save_depth_maps:
                        ...
                        current_depth_value = depth_map[discretized_projected_point[1],
                                                        discretized_projected_point[0]]
                        candidate_depth_value = self.depth_factor * \
                            np.sqrt(np.sum(np.square(point_in_3D)))
                        if current_depth_value == 0:
                            depth_map[discretized_projected_point[1],
                                      discretized_projected_point[0]] = candidate_depth_value
            ...
            if self.save_depth_maps:
                dir_depth = os.path.join(
                    self.dir_d, str(str(self.counter)+".png"))
                ...
                dir_img = os.path.join(
                    self.dir_control, str(str(self.counter) + ".png"))
                cv2.imwrite(dir_img, cv_image)
            ...
            if self.publish_colourised_pcd_msgs:
                ...
                ros_point_cloud_msg = point_cloud2.create_cloud(
                    header, fields, lidar_colourized_points)
    ...
    def get_computation_extrinsic_mtx(rot, trans):
        ...
        my_transformation_mtx = np.append(
            rot_mat, [[trans[0]], [trans[1]], [trans[2]]], 1)
        my_transformation_mtx = np.append(
            my_transformation_mtx, [[0., 0., 0., 1.]], 0)
        ...
        while trans is None:
            try:
                (trans, rot) = tf_listener.lookupTransform(
                    '/velodyne', camera_tf, rospy.Time(0))
                ...
                if counter_local == 5000:
                    text = "Looking for TF between /velodyne and {} ...".format(
                        camera_tf)
                    rospy.logwarn(text)
                    counter_local = 0
                ...
                if counter_global == 500000:
                    text = "TF between /velodyne and {} not found.".format(
                        camera_tf)
                    rospy.logerr(text)
                    raise RuntimeError(text)
...
def main(args):
    ...
    path2cam_calib_yaml = str(rospy.get_param(
        "/projector2/path2cam_calib_yaml"))
    sync_difference_time = float(rospy.get_param(
        "/projector2/sync_difference_time"))
    img_topic2sub = str(rospy.get_param("/projector2/subscriber_img_topic"))
    lidar_topic2sub = str(rospy.get_param(
        "/projector2/subscriber_lidar_topic"))
    ...
    save_images_and_lidar_bags = rospy.get_param(
        "/projector2/save_images_and_lidar_bags")
    create_control = rospy.get_param("/projector2/create_control")
    input_images_are_distorted = bool(rospy.get_param(
        "/projector2/input_images_are_distorted"))
    depth_factor = float(rospy.get_param("/projector2/depth_factor"))
    publish_colourised_pcd_msgs = bool(rospy.get_param(
        "/projector2/publish_colourised_pcd_msgs"))
